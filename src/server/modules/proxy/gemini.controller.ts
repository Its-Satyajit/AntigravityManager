...
export class GeminiController {
  ...
  listModels(@Res() res: FastifyReply) {
	...
	getModel(@Param('model') model: string, @Res() res: FastifyReply) {
		...
		const matched = this.buildGeminiModelList().find(
	...
	async modelAction(
		...
		await this.handleModelActionDispatch(
			parsed.model,
			parsed.action,
			body,
			res,
	...
	private async handleModelActionDispatch(
		...
		try {
			if (action === "streamGenerateContent") {
				const stream =
					await this.proxyService.handleGeminiStreamGenerateContent(
						model,
						body,
					);
			...
			if (action === "generateContent") {
				...
				res
					...
					.send(this.buildNormalizedGeminiGenerateResponse(result));
				return;
			}
	...
	private parseModelActionToken(modelAction: string): {
		...
		const prefixedModel = model.startsWith("models/")
			? model
			: `models/${model}`;
		return {
			model: prefixedModel,
	...
	private buildGeminiModelList(): GeminiModelMetadata[] {
		...
		return dynamicModelIds.map((id) =>
			this.toGeminiModelMetadata(`models/${id}`),
		);
	}
	...
	private buildNormalizedGeminiGenerateResponse(
		...
		if (response.usageMetadata) {
			const usageMetadata = this.normalizeGeminiUsageMetadata(
	...
	private normalizeGeminiUsageMetadata(
		...
		if (usageMetadata?.candidatesTokensDetails !== undefined) {
			normalized.candidatesTokensDetails =
	...
	private supportsReplyHijack(
		reply: FastifyReply,
	): reply is FastifyReply & { hijack: () => void } {
