...
export class ProxyController {
    ...
    @Optional() private readonly tokenManager?: TokenManagerService,
  ) {}
  ...
  listModels(@Res() res: FastifyReply) {
      ...
      stream?: boolean;
		...
		try {
			...
			res
				...
				.send(this.toLegacyTextCompletionsResponse(response));
		} catch (error) {
	...
	private normalizeCompletionPrompt(
	...
	private toLegacyTextCompletionsResponse(
		...
		if (inputItems) {
			for (const item of inputItems) {
				...
				if (
					...
					const callId =
						this.asString(itemObj.call_id) ??
						this.asString(itemObj.id) ??
						`call_${Date.now()}`;
					const toolName =
						type === "local_shell_call"
							? "shell"
			...
			for (const item of inputItems) {
				...
				if (type === "message") {
					...
					const content = this.normalizeResponsesMessageContent(
						itemObj.content,
					);
	...
	private normalizeResponsesMessageContent(
		content: unknown,
	...
	private resolveInlineData(input: unknown): {
		...
		if (isString(input)) {
			const dataUri = input.match(
				/^data:(?<mime>[^;]+);base64,(?<data>[A-Za-z0-9+/=]+)$/,
			);
	...
	private async sendOpenAIImageGenerationResponse(
		...
		try {
			...
			const image = this.extractInlineBase64Image(
			...
			if (this.isProjectContextErrorMessage(message)) {
				try {
					...
					const geminiResult =
						await this.proxyService.handleGeminiGenerateContent(
							request.model ?? "gemini-3-pro-image",
							geminiRequest,
						);
					const fallbackImage =
						this.extractInlineBase64ImageFromGeminiResponse(geminiResult);
					if (fallbackImage) {
						res.status(HttpStatus.OK).send({
							created: Math.floor(Date.now() / 1000),
							data: [
	...
	private extractInlineBase64ImageFromGeminiResponse(
	...
	private buildGeminiImageRequest(
		...
		if (!userMessage) {
			parts.push({
				text:
					fallbackPrompt || "Please generate an image based on this request.",
			});
	...
	private isProjectContextErrorMessage(message: string): boolean {
		...
		return (
			...
			(lowered.includes("google cloud project") &&
				lowered.includes("code assist license")) ||
			(lowered.includes("resource projects/") &&
	...
	private hasMultipartBoundary(req: FastifyRequest): boolean {
		...
		return (
			lowered.includes("multipart/form-data") && lowered.includes("boundary=")
		);
	}
	...
	private resolveErrorHttpStatus(message: string): HttpStatus {
		...
		if (
			lowered.includes("all accounts exhausted") ||
			lowered.includes("no available accounts")
		) {
